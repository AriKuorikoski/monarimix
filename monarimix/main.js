/*
  REFERENCE COPY - from REAPER's stock installation
  Location: C:\Program Files\REAPER (x64)\Plugins\reaper_www_root\main.js

  This is NOT deployed by deploy.ps1. REAPER's web server automatically overlays
  the user's reaper_www_root/ over the install copy, so the stock main.js is
  always available at <script src="main.js"> in monarimix.html.

  This file is included in the repo as reference documentation only, so developers
  can see what wwr_* functions are available and how they work.

  See CLAUDE.md > Key Architecture Concepts > Communication Protocol with REAPER
  for details on how monarimix uses these functions.
*/
